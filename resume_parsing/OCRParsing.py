...
class OCRParsing:
    ...
    def createBoxes(self, bounds):
        ...
        old_bounds = []
        for x in bounds:
            old_bounds.append(x[1].lower())
            if x[1].lower() in categories or self.is_education_section(x[1].lower()):
                box.append(x)
        print("old bounds", old_bounds)
        return box
    ...
    def giveProperNames(self, new_bounds):
        ...
        for x in new_bounds:
            text = x[1].lower()
            new_bounds.append(text)
            if len(properNameBounds) == 0:
                properNameBounds.append(x)
            elif text in educationNames or self.is_education_section(text):
    ...
    def is_education_section(self, text):
        # Regular expressions to identify education sections with various capitalizations and OCR errors
        education_keywords = [
        
            r'\b[Ee]ducation[a-z]*\b',  # Matches "Education" or "education" with any trailing characters
            r'\bEDUCATION[A-Z]*\b',  # Matches "EDUCATION" in all caps with any trailing uppercase characters
        
        ]
        return any(re.search(keyword, text, re.IGNORECASE) for keyword in education_keywords)
